 ...
 * - **Production**: off unless ``localStorage.setItem('erza_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('erza_debug_ws','0')`` (or ``false`` / ``off``).
...
export interface ErzaClientOptions {
  url: string;
  reconnect?: boolean;
...
export class ErzaClient {
  ...
  constructor(private options: ErzaClientOptions) {
  ...
  sendMessage(
    ...
    this.knownChats.add(chatId);
    const metadata: OutboundMessageMetadata | undefined = options?.agentId
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[erza ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - ErzaClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
