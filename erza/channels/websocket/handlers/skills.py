...
def list_skills(ctx: RouteContext) -> Response:
    ...
    try:
        ...
        for entry in all_entries:
            ...
            always = bool(
                loader._parse_erza_metadata(meta.get("metadata")).get("always")
                or meta.get("always")
            )
...
def save_skill(ctx: RouteContext) -> Response:
    ...
    Accepts the new content via repeated ``x-erza-Skill-Content``
    ...
    header_b64 = _collect_chunked_header(ctx.request.headers, "x-erza-Skill-Content")
    if header_b64:
...
def upload_skill(ctx: RouteContext) -> Response:
    ...
    transported as base64 chunks in repeated ``x-erza-Skill-Zip``
    ...
    if not b64_data:
        return _http_error(400, "missing ZIP data (send via x-erza-Skill-Zip headers)")
