...
def save(ctx: RouteContext) -> Response:
    ...
    Accepts content via repeated ``x-erza-Agent-Content`` headers
    ...
    header_data = _collect_chunked_header(ctx.request.headers, "x-erza-Agent-Content")
    if header_data:
...
async def generate_agent(ctx: RouteContext) -> Response:
    ...
    ``x-erza-Agent-Description`` chunked header (URL-encoded)
    ...
    header_data = _collect_chunked_header(ctx.request.headers, "x-erza-Agent-Description")
    if header_data:
