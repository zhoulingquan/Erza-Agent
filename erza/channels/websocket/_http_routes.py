...
from erza.channels.websocket._chunked_header import (  # noqa: F401 — re-exported for channel.py
...
_MCP_VALUES_HEADER = "x-erza-MCP-Values"
...
_SENSITIVE_VALUES_HEADER = "x-erza-Values"
...
def _merge_sensitive_values_header(
    ...
    """把 ``x-erza-Values`` chunked JSON header 合并进 *query*。
...
def _issue_route_secret_matches(headers: Any, configured_secret: str) -> bool:
    ...
    if authorization and authorization.lower().startswith("bearer "):
        ...
        return constant_time_equals(supplied, configured_secret)
    header_token = headers.get("x-erza-Auth") or headers.get("x-erza-auth")
    if not header_token:
        return False
    return constant_time_equals(header_token.strip(), configured_secret)
