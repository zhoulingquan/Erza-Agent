...
def _resolve_context_window_for_settings(model: str, configured: int | None) -> dict[str, Any]:
    ...
    try:
        from erza.providers.model_catalog import _load_learned_entry, _normalize_model_name
...
def _trigger_model_learning(model: str) -> dict[str, Any]:
    ...
    if not model:
        return {"limit": 65_536, "status": "default", "error": None}
    try:
        from erza.providers.model_catalog import learn_model_context_limit
    ...
    try:
        from erza.providers.model_catalog import _save_learned_failure
