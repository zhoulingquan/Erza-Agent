...
from erza.security.network import validate_resolved_url, validate_url_target
...
class ErzaDingTalkHandler(CallbackHandler):
    """
    Standard DingTalk Stream SDK Callback Handler.
    Parses incoming messages and forwards them to the erza channel.
    """
    ...
    async def process(self, message: CallbackMessage):
        ...
        try:
            ...
            # Forward to erza via _on_message (non-blocking).
...
class DingTalkChannel(BaseChannel):
    ...
    # _IMAGE_EXTS / _AUDIO_EXTS / _VIDEO_EXTS 由 erza.channels._media_common
    # 提供（各频道取最大并集），模块级导入后直接使用，不再在类里重复定义。
    ...
    async def start(self) -> None:
        ...
        try:
            if not DINGTALK_AVAILABLE:
                self.logger.error(
                    "Stream SDK not installed. Run: erza plugins enable dingtalk"
                )
                return
            ...
            handler = ErzaDingTalkHandler(self)
    ...
    async def _on_message(
        ...
        """Handle incoming message (called by ErzaDingTalkHandler).
