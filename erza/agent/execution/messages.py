"""Agent-execution message building & per-turn throttling helpers.

Same execution domain as ``TurnRecoveryPolicy`` (``recovery.py``): prompt
snippets, retry/length-recovery/goal-continue message builders, empty-tool
result markers and the repeated-external-lookup throttle.  Moved here from
``erza.utils.runtime`` (which keeps the shared ``EMPTY_FINAL_RESPONSE_MESSAGE``
and the workspace-violation throttle that tests/tools still import from
``erza.utils.runtime``).
"""
...
from erza.utils.helpers import stringify_text_blocks
