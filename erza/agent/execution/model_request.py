...
class ModelRequestExecutor:
    ...
    async def request_model(
        ...
        if timeout_s is None:
            ...
            # Set ERZA_LLM_TIMEOUT_S=0 to disable.
            raw = os.environ.get("ERZA_LLM_TIMEOUT_S", "300").strip()
            try:
                timeout_s = float(raw)
        ...
        # (ERZA_STREAM_IDLE_TIMEOUT_S). Do not also apply the outer wall-clock
        # LLM timeout here, or healthy long reasoning streams can be killed just
        # because total elapsed time exceeded ERZA_LLM_TIMEOUT_S.
