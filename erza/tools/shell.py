...
from erza.security.workspace_policy import is_path_within
...
from erza.tools.sandbox import wrap_command
...
# Prefer PowerShell 7 (`pwsh`) on Windows; fall back to the legacy 5.1
# `powershell.exe` only if 7 is not installed.
_WINDOWS_POWERSHELL: str | None = None


def _windows_powershell() -> str:
    """Return the Windows PowerShell binary to use (PowerShell 7 preferred)."""
    global _WINDOWS_POWERSHELL
    if _WINDOWS_POWERSHELL is None:
        _WINDOWS_POWERSHELL = shutil.which("pwsh") or shutil.which("powershell") or "powershell"
...
class ExecTool(Tool):
    ...
    def __init__(
        ...
        self.deny_patterns = (
            ...
            + [
                ...
                r":\(\)\s*\{.*\};\s*:",  # fork bomb
                # Block writes to Erza internal state files (#2989).
    ...
    def _prepare_command(
        ...
        if self.path_append:
            if _IS_WINDOWS:
                ...
                env["ERZA_PATH_APPEND"] = self.path_append
                command = f'export PATH="$PATH{os.pathsep}$ERZA_PATH_APPEND"; {command}'
    ...
    def _guard_command(
        ...
        *,
        ...
        from erza.security.network import contains_internal_url
        ...
        if should_restrict:
            ...
            from erza.security.workspace_policy import expand_env_vars
