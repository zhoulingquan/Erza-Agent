...
from erza.security.workspace_access import current_tool_workspace
from erza.tools.base import Tool, tool_parameters
from erza.tools.file_state import FileStates, _hash_file, current_file_states
from erza.tools.path_utils import resolve_workspace_path, verify_workspace_path
from erza.tools.receipts import ToolReceiptClaim, emit_receipt
...
from erza.utils.helpers import build_image_content_blocks, detect_image_mime
...
def _is_blocked_device(path: str | Path) -> bool:
    ...
    if resolved.startswith("/dev/"):
        ...
        if resolved_name in _BLOCKED_DEVICE_PATHS:
            return True
        try:
            if Path(resolved).is_block_device() or Path(resolved).is_char_device():
                return True
        except OSError:
            return True
    return False
...
class ReadFileTool(_FsTool):
    ...
    def _read_office_doc(self, fp: Path) -> str:
        from erza.utils.document import extract_text
