...
module's namespace (e.g. ``patch("erza.cli.commands.PromptSession",
...)`` or ``patch("erza.cli.commands.evaluate_response", ...)``).
...
(``from erza.cli import commands; commands.<name>(...)``).
...
# Remove default handler and re-add with unified Erza format
logger.remove()
...
from erza.agent.loop import AgentLoop  # noqa: F401 — re-exported for late binding
from erza.cli._gateway_runner import _run_gateway
from erza.cli._terminal_render import (
...
from erza.cli.stream import StreamRenderer, ThinkingSpinner
...
app = typer.Typer(
    ...
    help=f"{__logo__} Erza - Personal AI Assistant",
    no_args_is_help=True,
)

# ---------------------------------------------------------------------------
# CLI input: prompt_toolkit session state (lives on the commands module so
# that test patches like `patch("erza.cli.commands._PROMPT_SESSION",
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} Erza v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize Erza configuration and workspace."""
    from erza.config.loader import get_config_path, load_config, save_config, set_config_path
    from erza.config.schema import Config
    ...
    if wizard:
        from erza.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print(
                "[yellow]Please run 'erza onboard' again to complete setup.[/yellow]"
            )
            raise typer.Exit(1)
    ...
    agent_cmd = 'erza agent -m "Hello!"'
    gateway_cmd = "erza gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} Erza is ready!")
    console.print("\nNext steps:")
    if wizard:
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from erza.config.loader import load_config, resolve_config_env_vars, set_config_path
...
def _warn_deprecated_config_keys(config_path: Path | None) -> None:
    ...
    from erza.config.loader import get_config_path
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show Erza runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print(
            r"[red]aiohttp is required. Install with: pip install erza-ai\[api][/red]"
        )
        raise typer.Exit(1)

    from loguru import logger

    from erza.api_compat.server import create_app
...
def _ensure_local_allow_from(cfg: Config) -> None:
    """For local dev (``erza gateway``), auto-allow all clients on the
...
def _load_or_create_desktop_config(config: str | None, workspace: str | None) -> Config:
    ...
    from erza.config.schema import Config as ErzaConfig
...
def agent(
    ...
    logs: bool = typer.Option(
        False, "--logs/--no-logs", help="Show Erza runtime logs during chat"
    ),
):
...
def channels_status(
    ...
    """Show channel status."""
    from erza.channels.registry import discover_all
    from erza.config.loader import load_config, set_config_path
...
def channels_login(
    channel_name: str = typer.Argument(
        ..., help="Channel name (use 'erza channels list' to see available)"
    ),
...
def status():
    ...
    console.print(f"{__logo__} Erza Status\n")
...
# commands 模块上的属性(例如 patch("erza.cli.commands.evaluate_response", ...)),
...
from erza.cli._terminal_render import (  # noqa: F401 — compat re-exports
    FileHistory,
    SafeFileHistory,
...
from erza.utils.evaluator import evaluate_response  # noqa: F401 — compat re-export
