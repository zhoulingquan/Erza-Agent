"""Skill contract shared by the agent core, the tools layer and the channels.

``BUILTIN_SKILLS_DIR`` and the full ``SkillsLoader`` moved from
``erza.agent.skills`` so resource consumers (tools/filesystem.py, the
WebSocket channel's ``/api/skills*`` handlers) can resolve the builtin
skills directory and load/validate skills without pulling in ``erza.agent``.

The loader lazily touches config at call time only (``_refresh_disabled_from_config``)
so this module's import-time surface stays stdlib-only (plus ``yaml``).

``erza.agent.skills`` re-exports these symbols; import from
``erza.contracts.skills`` in new code.
"""
...
# 默认内置技能目录；与历史路径保持一致（仓库根 skills/，曾由 erza.agent.skills 定义）。
...
class SkillsLoader:
    ...
    def _parse_erza_metadata(self, raw: object) -> dict:  # noqa: N802
        """Extract Erza/openclaw metadata from a frontmatter field.
        ...
        if not isinstance(data, dict):
            return {}
        payload = data.get("erza", data.get("Erza", data.get("openclaw", {})))
    ...
    def _get_skill_meta(self, name: str) -> dict:
        """Get Erza metadata for a skill (cached in frontmatter)."""
        raw_meta = self.get_skill_metadata(name) or {}
        return self._parse_erza_metadata(raw_meta.get("metadata"))
    ...
    def get_always_skills(self) -> list[str]:
        ...
        return [
            ...
            and (
                self._parse_erza_metadata(meta.get("metadata")).get("always") or meta.get("always")
            )
        ]
