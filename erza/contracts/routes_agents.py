"""Route handlers for declarative subagent management (contract shared layer).
...
server lives in ``erza.api_compat`` and does NOT re-export this
module, keeping the websocket channel free of any aiohttp dependency.

The LLM-backed generator path stays generic: ``generate_agent`` resolves
``erza.agent.agent_generator`` lazily at call time so this contract module's
import-time surface stays free of ``erza.agent``.

``erza.agent.routes_agents`` re-exports ``router`` for backward
compatibility; import from ``erza.contracts.routes_agents`` in new code.
"""
