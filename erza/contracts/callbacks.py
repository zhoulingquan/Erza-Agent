...
``ProgressCallback`` 定义在这里而不是 ``progress`` (后者 import
agent.hook, 会被 agent 包初始化链反向依赖形成环), 保证任何模块都能安全
先行导入。原住址 ``erza.utils.callback_types`` (which now re-exports).
"""
...
#   (见 session.progress._tool_events 的 on_progress_accepts_*);
