...
(``erza.bus.queue.MessageBus``, ``erza.cron.service.CronService``,
``erza.session.manager.SessionManager``,
``erza.providers.factory.build_provider_snapshot``,
``erza.cli.commands.AgentLoop`` …) keep taking effect — the same
...
from erza.cli._terminal_render import console
...
class GatewayApplication:
    ...
    def __init__(
        ...
        *,
        ...
        from erza.channels.websocket import publish_runtime_model_update
        ...
        console.print(
            f"{__logo__} Starting Erza gateway version {__version__} on port {ws_port}..."
        )
    ...
    def _pick_heartbeat_target(self) -> tuple[str, str]:
        from erza.cli._gateway_runner import _pick_heartbeat_target as _pick_target

        return _pick_target(self.channels, self.session_manager)

    def _on_cron_job(self, job: CronJob) -> "Any":
        from erza.cli._gateway_runner import on_cron_job
    ...
    def _reload_cron_system_jobs(self) -> None:
        ...
        gateway restart.
        """
        from erza.config.loader import load_config as _reload_config
